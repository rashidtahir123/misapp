...
class ServiceListing extends Component<{}> {
  ...
  loadListing = async () =>{
       ...
       if(state.isConnected ==true){
         this.offset = 0;
          fetch(global.serverURL+'front/api/getServiceApi?search=2&offset=' + this.offset+'&empid='+userDetails.EmployeeID)
       ...
       fetch(global.serverURL+'front/api/getTotalRecord?type=3&empid='+userDetails.EmployeeID, {
           headers: {
  ...
  servicedetail = (tikno) => {
      ...
      Actions.servicedetail({ ticketID: tikno });
    }
    onSearch = (searchText) => {
      const {getUser: {userDetails}} = this.props;
          return new Promise((resolve, reject) => {
            //  console.log(searchText);
              ...
              fetch(global.serverURL+'front/api/getServiceApi?search=1&offset=0&empid='+userDetails.EmployeeID+"&ticketref="+searchText)
                      .then(response => response.json())
                      .then(responseJson => {
                          this.setState({ serverData:[]});
              ...
              renderItem={({ item, index }) => (
                <TouchableHighlight key={item.TicketID.toString()}  style={{marginLeft:10,marginRight:10}}>
                    <CardItem  style={styles.ticketListing}>
                      <Body>
                        <Grid>
                          <Col  style={{ marginLeft:5,marginRight:5}} onPress={() => this.servicedetail(item.TicketID)}>
