...
class MaintenanceListing extends Component<{}> {
  ...
  async componentDidMount() {
   ...
   loadListing = async () =>{
       ...
       if(state.isConnected ==true){
         this.offset = 0;
          fetch(global.serverURL+'front/api/getMaintenanceApi?search=2&offset=' + this.offset+'&empid='+userDetails.EmployeeID)
       ...
       fetch(global.serverURL+'front/api/getTotalRecord?type=5&empid='+userDetails.EmployeeID, {
           headers: {
     ...
     NetInfo.fetch().then(state => {
           if(state.isConnected ==true){
              ...
              .then(responseJson => {
                   ...
                   this.setState({ serverData: [...this.state.serverData, ...responseJson],loading: false,showingRec:nextTotal});
  ...
  onSearch = (searchText) => {
    const {getUser: {userDetails}} = this.props;
        return new Promise((resolve, reject) => {
            ...
            fetch(global.serverURL+'front/api/getMaintenanceApi?search=1&offset=0&empid='+userDetails.EmployeeID+"&ticketref="+searchText)
                    .then(response => response.json())
                    .then(responseJson => {
                        this.setState({ serverData:[]});
  ...
  maintenancedetail = (tikno) => {
  //  Alert.alert(ref);
      Actions.maintenancedetail({ ticketID: tikno });
    }
              ...
              renderItem={({ item, index }) => (
                <TouchableHighlight key={item.TicketID.toString()}   style={{marginLeft:10,marginRight:10}}>
                    <CardItem  style={styles.ticketListing}>
                      <Body>
                        <Grid>
                          <Col  style={{ marginLeft:5,marginRight:5}} onPress={() => this.maintenancedetail(item.TicketID)}>
...
export default connect(mapStateToProps, mapDispatchToProps)(MaintenanceListing);
