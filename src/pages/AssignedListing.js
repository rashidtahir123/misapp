...
class AssignedListing extends Component<{}> {
  ...
  async componentDidMount() {
   ...
   loadMoreData = async  () => {
      ...
      if(state.isConnected ==true){
         fetch(global.serverURL+'front/api/getAssignedTickets?search=2&offset=' + this.offset+'&empid='+userDetails.EmployeeID)
      .then(response => response.json())
             .then(responseJson => {
              ...
              if(count >0){
                ...
                this.setState({serverData: [...this.state.serverData, ...responseJson],showingRec:nextTotal});
         ...
         NetInfo.fetch().then(state => {
           if(state.isConnected ==true){
             ...
             .then(responseJson => {
              ...
              fetch(global.serverURL+'front/api/getTotalAssignedTickets?type=10&empid='+userDetails.EmployeeID, {
                  headers: {
  ...
  assigneddetail = (tikno) => {
      Actions.assigneddetail({ ticketID: tikno });
  }
  onSearch = (searchText) => {
    const {getUser: {userDetails}} = this.props;
        return new Promise((resolve, reject) => {
          //  console.log(searchText);
            ...
            fetch(global.serverURL+'front/api/getAssignedTickets?search=1&offset=0&empid='+userDetails.EmployeeID+"&ticketref="+searchText)
                    .then(response => response.json())
                    .then(responseJson => {
    ...
    render() {
      ...
      alignItems:'center',flex:1}}/>) :(<FlatList
              ...
              keyExtractor={item => item.AssignTicketID}
              renderItem={({ item, index }) => (
                <TouchableHighlight key={item.AssignTicketID.toString()}  style={{marginLeft:10,marginRight:10}}>
                    <CardItem  style={styles.ticketListing}>
                      <Body>
                        <Grid>
                          <Col style={{ marginLeft:5,marginRight:5}}  onPress={() => this.assigneddetail(item.AssignTicketID)}>
...
export default connect(mapStateToProps, mapDispatchToProps)(AssignedListing);
